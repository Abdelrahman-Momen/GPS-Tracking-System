...
void MCAL_UART_CONFIG(UART_Select UART_NO, uint8_t DATA_BITS, uint8_t PARITY, uint8_t STOP_BITS, uint32_t BAUD_RATE, uint8_t CLK_DIV){
    ...
    switch (UART_NO) {
        case UART0:
            ...
            MCAL_UART_INIT(UART0);
            UART0_CTL &= ~UARTEN;                                   //disable UART
            MCAL_UART_SETBRD(BAUD_RATE, CLK_DIV);
            UART0_IBRD &= ~(uint32_t)IBRD;
            UART0_IBRD |= (uint32_t)IBRD;                           //baud rate calc.
            
            UART0_FBRD &= ~(uint32_t)FBRD;
            UART0_FBRD |= (uint32_t)FBRD;
            
            UART0_LCRH &= ~((DATA_BITS - 5) << UART_LCRH_WLEN);
...
//*************************************** <END OF UART0> *********************************************
        ...
        case UART1:
            ...
            MCAL_UART_SETBRD(BAUD_RATE, CLK_DIV);
            UART1_IBRD &= ~(uint32_t)IBRD;
            UART1_IBRD |= (uint32_t)IBRD;                           //baud rate calc.
            
            UART1_FBRD &= ~(uint32_t)FBRD;
            UART1_FBRD |= (uint32_t)FBRD;
            
            UART1_LCRH &= ~((DATA_BITS - 5) << UART_LCRH_WLEN);
        ...
        case UART2:
            ...
            MCAL_UART_INIT(UART2);
            UART2_CTL &= ~UARTEN;                                   //disable UART
            MCAL_UART_SETBRD(BAUD_RATE, CLK_DIV);
            UART2_IBRD &= ~(uint32_t)IBRD;
            UART2_IBRD |= (uint32_t)IBRD;                           //baud rate calc.
            
            UART2_FBRD &= ~(uint32_t)FBRD;
            UART2_FBRD |= (uint32_t)FBRD;
            
            UART2_LCRH &= ~((DATA_BITS - 5) << UART_LCRH_WLEN);
        ...
        case UART3:
            ...
            MCAL_GPIO_Init(PORTC);
            MCAL_UART_INIT(UART3);
            UART3_CTL &= ~UARTEN;                                   //disable UART
            MCAL_UART_SETBRD(BAUD_RATE, CLK_DIV);
            UART3_IBRD &= ~(uint32_t)IBRD;
            UART3_IBRD |= (uint32_t)IBRD;                           //baud rate calc.
            
            UART3_FBRD &= ~(uint32_t)FBRD;
            UART3_FBRD |= (uint32_t)FBRD;
            
            UART3_LCRH &= ~((DATA_BITS - 5) << UART_LCRH_WLEN);
...
//*************************************************** <END OF UART3> ************************************************************
        ...
        case UART4:
            ...
            MCAL_GPIO_Init(PORTC);
            MCAL_UART_INIT(UART4);
            UART4_CTL &= ~UARTEN;                                   //disable UART
            MCAL_UART_SETBRD(BAUD_RATE, CLK_DIV);
            UART4_IBRD &= ~(uint32_t)IBRD;
            UART4_IBRD |= (uint32_t)IBRD;                           //baud rate calc.
            
            UART4_FBRD &= ~(uint32_t)FBRD;
            UART4_FBRD |= (uint32_t)FBRD;
            
            UART4_LCRH &= ~((DATA_BITS - 5) << UART_LCRH_WLEN);
            UART4_LCRH |= (DATA_BITS - 5) << UART_LCRH_WLEN;        //set data format         //set data format
...
//*************************************************** <END OF UART4> ************************************************************
        ...
        case UART5:
            ...
            MCAL_UART_INIT(UART5);
            UART5_CTL &= ~UARTEN;                                   //disable UART
            MCAL_UART_SETBRD(BAUD_RATE, CLK_DIV);
            UART5_IBRD &= ~(uint32_t)IBRD;
            UART5_IBRD |= (uint32_t)IBRD;                           //baud rate calc.
            
            UART5_FBRD &= ~(uint32_t)FBRD;
            UART5_FBRD |= (uint32_t)FBRD;
            
            UART5_LCRH &= ~((DATA_BITS - 5) << UART_LCRH_WLEN);
            UART5_LCRH |= (DATA_BITS - 5) << UART_LCRH_WLEN;        //set data format         //set data format
...
//************************************************** <END OF UART5> *************************************************************
        ...
        case UART6:
            ...
            MCAL_GPIO_Init(PORTD);
            MCAL_UART_INIT(UART6);
            UART6_CTL &= ~UARTEN;                                   //disable UART
            MCAL_UART_SETBRD(BAUD_RATE, CLK_DIV);
            UART6_IBRD &= ~(uint32_t)IBRD;
            UART6_IBRD |= (uint32_t)IBRD;                           //baud rate calc.
            
            UART6_FBRD &= ~(uint32_t)FBRD;
            UART6_FBRD |= (uint32_t)FBRD;
            
            UART6_LCRH &= ~((DATA_BITS - 5) << UART_LCRH_WLEN);
            UART6_LCRH |= (DATA_BITS - 5) << UART_LCRH_WLEN;        //set data format         //set data format
...
//*************************************************** <END OF UART6> ************************************************************
        ...
        case UART7:
            ...
            MCAL_GPIO_Init(PORTE);
            MCAL_UART_INIT(UART7);
            UART7_CTL &= ~UARTEN;                                   //disable UART
            MCAL_UART_SETBRD(BAUD_RATE, CLK_DIV);
            UART7_IBRD &= ~(uint32_t)IBRD;
            UART7_IBRD |= (uint32_t)IBRD;                           //baud rate calc.
            
            UART7_FBRD &= ~(uint32_t)FBRD;
            UART7_FBRD |= (uint32_t)FBRD;
            
            UART7_LCRH &= ~((DATA_BITS - 5) << UART_LCRH_WLEN);
            UART7_LCRH |= (DATA_BITS - 5) << UART_LCRH_WLEN;        //set data format         //set data format
